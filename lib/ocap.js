...
function OCAP( userAgent, isDebugging ) {
  ...
  self.lockBasket = function ( connectionId, connection, callback, eventId, supplierId, basketId, statusCode ) {
    if ( connection ) { 
      if ( connection.user_type == Config.BUYER ) {
        ...
        self.__post( payload, 'events/' + eventId + '/' + supplierId + '/basket/' + basketId + '/status',
          function ( error ) {
            if ( !error )
  ...
  self.__get = function ( route, err, resp, instance, cookie ) {
    if ( isDebugging )
      console.log(Config.debugPrefix + ' GET ' + self.protocol + instance + '.pollenware.com/services/' + route);
    request(
      {
        'method' : 'get',
  ...
  self.__post = function ( payload, route, err, resp, instance, cookie ) {
    if ( isDebugging ) {
      ...
      console.log(Config.debugPrefix + ' POST ' + self.protocol + instance + '.pollenware.com/services/' + route);
    }
    request(
      {
        'method' : 'post',
