var Config = require( '../../config/ocap.js' ).Config,
    Net    = require( './net.js' ).Net,
    Util   = require( '../util.js' ).Util,
    request = require( '../third-party/request.js' );

exports.OCAP = OCAP;

function OCAP( userAgent ) {

  var self = this;

  self.net = new Net();

  self.lockBasket = function ( connectionId, connection, callback, eventId, supplierId, basketId, statusCode ) {
    if ( connection ) { 
      if ( connection.user_type == Config.BUYER ) {
        ...
        console.log( Config.errorSupplierCant );
      }
    }
    else {
  ...
  self.isDebugging = function ( turnOn ) {
    if ( turnOn ) {
  ...
  self.loginUser = function ( connectionId, connection, postCommandHandler ) {

    var loginCallback = function ( connectionId, error ) {
      if ( error ) {
        console.log( Config.errorConnect + ': ' + connectionId );
        delete self.connections[connectionId]; 
        postCommandHandler( Config.promptPrefix );
        return;
      }
      else {
        self.getUserDetails( connectionId, connection, function () {loginCallback( connectionId )} );
      }
    };
    ...
    self.net.post( payload, 'login',
      function ( error ) {
        ...
        connection.lastAction = Config.failedLoginMsg;
        console.error( Config.errorPrefix + ' ' + error );
        loginCallback( connectionId, error );
      },
      function ( response ) {
  ...
  self.__getInvoices = function ( connectionId, connection, callback, userType, query ) {
    ...
    self.net.get(  'invoices/' + userType + '/' + connection.org_id + self.__marshalInvoiceQuery( connection, query ),
      function ( error ) {
        if ( !error )
          error = Config.networkError;
        connection.lastAction = Config.failedLoginMsg;
