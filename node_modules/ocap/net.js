    ...
    request( 
      ...
      function ( error, response, body ) {
        if ( !error && response.statusCode == 200 ) {
          ...
          function onTickFail() {
            try {
              emitter.emit( 'failedOCAPresponse', ( response && ( response.statusCode + MCat.s + error ) ) || error );
            }
            catch( e ) {
              emitter.emit( 'failedOCAPfailed', e );
              return;
            }
          }
    ...
    var onTick = function ( error, response, body ) {
      try {
        ...
        if ( ocap.payload.response.error ) {
          emitter.emit( 'failedOCAPresponse', ocap.payload.response.error_msg );
          return;
        }
        else {
  ...
  self.getReport = function ( route, responseCallback, hostname, cookie ) {
    var onTick = function ( error, response, body ) {
      try {
        ...
        emitter.emit( 'failedOCAPresponse', ( response && response.statusCode ) || e );
        return;
      }
    };
    self.__get( route, responseCallback, hostname, cookie, onTick );
    ...
    request(
      ...
      function ( error, response, body ) {
        if ( !error && response.statusCode == 200 ) {
          if ( response.body ) {
            var onTick = function () {
              try {
                ...
                if ( ocap.payload.response.error ) {
                  emitter.emit( 'failedOCAPresponse', null, ocap.payload.response.error_msg );
                  return;
                }
                else {
                  responseCallback( response );
          ...
          function onTick() {
            try {
              emitter.emit( 'failedOCAPresponse', null, response && response.statusCode );
            }
            catch( e ) {
              emitter.emit( 'failedOCAPfailed', null, e );
              return;
            }
          }
          process.nextTick( onTick )
