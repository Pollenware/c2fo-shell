    ...
    request( 
      ...
      function ( error, response, body ) {
        if ( !error && response.statusCode == 200 ) {
          ...
          function onTickFail() {
            try {
              emitter.emit( 'failOCAPresponse', ( response && ( response.statusCode + MCat.s + error ) ) || error );
            }
            catch( e ) {
  ...
  self.getReport = function ( route, responseCallback, hostname, cookie ) {
    var onTick = function ( error, response, body ) {
      try {
        ...
        emitter.emit( 'failOCAPresponse', ( response && response.statusCode ) || e );
        return;
      }
    };
    self.__get( route, responseCallback, hostname, cookie, onTick );
    ...
    request(
      ...
      function ( error, response, body ) {
        if ( !error && response.statusCode == 200 ) {
          if ( response.body ) {
            var onTick = function () {
              try {
                ...
                if ( ocap.payload.response.error ) {
                  emitter.emit( 'failOCAPresponse', null, ocap.payload.response.error_msg );
                  return;
                }
                else {
                  responseCallback( response );
                ...
                emitter.emit( 'failJSONParse', null, e );
                return;
              }
            }
          ...
          function onTick() {
            try {
              emitter.emit( 'failOCAPresponse', null, response && response.statusCode );
            }
            catch( e ) {
              emitter.emit( 'failOCAPfail', null, e );
              return;
            }
          }
          process.nextTick( onTick )
