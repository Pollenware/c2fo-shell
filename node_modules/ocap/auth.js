var MCat = require( 'message-catalog/ocap/auth.js' ).MCat;
...
function Auth( net, finishHandler, emitter ) {
  ...
  self.getDetails = function ( connectionId, connection, callback ) {
    ...
    self.net.get( 'auth/manage',
      ...
      function ( response ) {
        ...
        if ( self.isDebugging )
          emitter.emit( 'debugMessage', MCat.debugPrefix + ' - ' + instance + ' - ' + MCat.userDetails );
  ...
  self.signIn = function ( connectionId, password ) {
    ...
    if ( self.isDebugging )
      emitter.emit( 'debugMessage', MCat.connect );
