var Config  = require( 'config/main.js' ).Config,
    Net     = require( 'ocap/net.js' ).Net,
    Util    = require( 'util.js' ).Util,
    request = require( 'third-party/request.js' );

exports.Library = Library;

function Library( userAgent, postCommandHandler ) {

  var self = this;

  self.connectionCount = 1;

  self.__postCommandHandler = postCommandHandler;

  self.getUserDetails = function ( connectionId, connection ) {
    var cId       = connectionId.split( '@' );
    var instance  = cId[1];
    var emulating = cId[2];
    if ( !connection || !( connection.sessionCookie ) ) {
      self.__postCommandHandler( connectionId );
      return;
    }
    self.net.get( 'auth/manage',
      function ( error ) {
        ...
        self.__postCommandHandler( connectionId, error );
      },
      function ( response ) {
        ...
        if ( connection.user_type == Config.SUPPLIER  || connection.user_type == Config.BUYER ) {
          self.getEventDetails( connectionId, connection );
        }
        else {
          self.__postCommandHandler( connectionId );
        }
  ...
  self.lockBasket = function ( connectionId, connection, eventId, supplierId, basketId, statusCode ) {
    ...
    var lockBasketCallback = function ( connectionId, error ) {
      if ( error ) {
        ...
        self.__postCommandCallback( Config.promptPrefix );
        return;
      }
      else {
  ...
  self.loginUser = function ( connectionId, password ) {
    ...
    self.net.post( payload, 'login',
      function ( error ) {
        if ( !error )
          error = Config.networkError;
        console.error( Config.errorPrefix + ' ' + error );
        self.loginUserCallback( connectionId, password, user, instance, null, error );
      },
      function ( response ) {
        self.loginUserCallback( connectionId, password, user, instance, response );
      },
      instance
    );
  };
  ...
  self.getEventDetails = function ( connectionId, connection ) {
    var cId      = connectionId.split( '@' );
    var instance = cId[1];
    if ( !connection || !( connection.sessionCookie ) ) {
      self.__postCommandHandler( connectionId );
  ...
  self.getInvoices = function ( connectionId, connection, query ) {
  ...
  self.toggleInvoice = function ( connectionId, connection, keep, exclude ) {
