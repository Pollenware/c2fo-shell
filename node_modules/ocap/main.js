...
function Library( userAgent, finishHandler ) {
  ...
  self.getEventsDetail = function ( connectionId, connection ) {
    ...
    self.net.get( 'events/' + userType + '/' + connection.org_id,
      ...
      function ( response ) {
        connection.lastAction = Config.eventsDetailAction;
    ...
    self.net.get( 'auth/manage',
      ...
      function ( response ) {
        ...
        if ( connection.user_type == Config.SUPPLIER  || connection.user_type == Config.BUYER ) {
          self.getEventsDetail( connectionId, connection );
        }
        else {
          self.__finishHandler( connectionId );
        }
  ...
  self.net = new Net( userAgent, self.isDebugging );
  ...
  self.signinUser = function ( connectionId, password ) {
    ...
    self.net.post( payload, 'login',
      function ( error ) {
        if ( !error )
          error = Config.networkError;
        console.error( Config.errorPrefix + ' ' + error );
        self.signinUserCallback( connectionId, password, user, instance, null, error );
      },
      function ( response ) {
        self.signinUserCallback( connectionId, password, user, instance, response );
      },
      instance
    );
  };

  self.toggleInvoices = function ( connectionId, connection, keep, exclude ) {
