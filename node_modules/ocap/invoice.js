var MCat  = require( 'message-catalog/ocap/invoice.js' ).MCat;
var util = require('util') ;
exports.Invoice = Invoice;

function Invoice( net, finishHandler, emitter, pageSize ) {
  ...
  self.get = function ( connectionId, connection, query ) {
    ...
    if ( connection ) { 
      ...
      emitter.emit( 'failedOCAPservice', null, MCat.failedNoConnection );
    }
    ...
    if ( self.isDebgging )
      emitter.emit( 'debug', MCat.invoiceRetri );

    self.net.getObject( 'invoices/' + userType + '/' + connection.org_id + self.__marshalQuery( connection, query ),

      function ( response ) {
        connection.lastAction = MCat.invoiceDetAction;
        if ( self.isDebgging )
          emitter.emit( 'debug', instance + MCat.s + MCat.invoiceDetail );
        ...
        //set counts
        connection.invoice_counters = {};
        connection.invoice_counters.total_count = payload.response.total_count;
        connection.invoice_counters.total_invoices_selected = payload.response.total_invoices_selected;
        connection.invoice_counters.total_invoices_amount = payload.response.total_invoices_amount;
        connection.invoice_counters.per_page = pageSize;
        connection.invoice_counters.page_count = invoiceCount;

          connection.invoices_local = {};
        if ( self.isDebgging )
          emitter.emit( 'debug', instance + MCat.s + MCat.invoiceCount );
        for ( i in invoices ) {
  ...
  self.toggle = function ( connectionId, connection, keep, exclude ) {
    var toggleCallback = function ( connectionId, error ) {
      var cId = connectionId.split('@');
      if ( error ) {
        console.error( cId[1] + ' - ' + MCat.errorToggle );
        //self.callback( connectionId );
        return;
      }
      else {
        if ( self.isDebgging )
          emitter.emit( 'debug', cId[1] + MCat.s + MCat.successToggle );
        finishHandler( connectionId );
        //self.callback( connectionId );
      }
    };
    ...
    if ( connection.user_type == MCat.BUYER ) {
      ...
      for (var tuple in keep) {
        if ( /\|/.test( keep[tuple] ) ) {
          ...
          console.error( MCat.errorAmbiguous + ': ' + keep[tuple] );
        }
      } 
      for (var tuple in exclude) {
        if ( /\|/.test( exclude[tuple] ) ) {
          ...
          console.error( MCat.errorAmbiguous + ': ' + exclude[tuple] );
        }
      } 
    }
    else if ( connection.user_type == MCat.SUPPLIER ) {
    ...
    if ( self.isDebugging )
      emitter.emit( 'debug', MCat.toggle );

    self.net.post( payload, 'invoices/' + userType + '/' + connection.org_id + '/invoices',
      function ( response ) {
        connection.lastAction = MCat.successToggle;
        if ( self.isDebgging )
          emitter.emit( 'debug', instance + MCat.s + MCat.successToggle );
        var payload  = JSON.parse( response.body ).payload;
        connection.lastActive  = new Date( payload.created );
  ...
  self.__marshalQuery = function ( connection, query ) {
    ...
    // cmd           internal       regex                 , canBeRange 
      ...
      'basket'   : [ 'event_participation_id', /^\d+$/   , false ],
    ...
    for ( var token in query ) {
      var pattern = /^([gbkamountdevsrOipl_\-]+)([<>=])([A-Za-z0-9_\-\.]+)$/;
      ...
      if ( matches && matches[0] ) {
        ...
        if ( !validCommands[cmd] || !value.match( validCommands[cmd][1] ) || ( ( op == '>' || op == '<' ) && !validCommands[cmd][2] ) ) {
          console.error( MCat.invalidToken + query[token] + MCat.skipping  );
        }
        else {
          if ( seen[cmd] )
            continue;
          ...
          if ( cmd == 'event' ) {
            ...
            if ( usingDateFilter ) {
              console.error( MCat.noEventFilter  );
            ...
            if ( usingEventFilter ) {
              console.error( MCat.noDateFilter  );
        ...
        console.error( MCat.invalidToken + query[token] );
      }
    }

    if(!usingPage)
      queryString = queryString + "start=0";

    queryString = queryString.replace( /\&$/,'' );

    //var start = self.__paginate( connection );
    //return '?count=' + pageSize + '&' + 'start=' + start( pageSize ) + '&' + queryString;
    return '?count=' + pageSize + '&'  + queryString;
  };

/*
  self.__placeholder = 0 - pageSize;
