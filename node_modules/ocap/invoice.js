...
function Invoice( net, finishHandler, pageSize ) {
  ...
  self.get = function ( connectionId, connection, query ) {
    var invoiceCallback = function ( connectionId, error ) {
      ...
      if ( error ) {
        console.error( cId[1] + ' - ' + MCat.errorInvoice );
    ...
    self.net.get( 'invoices/' + userType + '/' + connection.org_id + self.__marshalQuery( connection, query ),
      ...
      function ( response ) {
        ...
        if ( self.isDebgging )
          console.info( instance   + ' - ' + MCat.invoiceDetailMsg );
    ...
    for ( var token in query ) {
      ...
      if ( matches && matches[0] ) {
        ...
        if ( !validCommands[cmd] || !value.match( validCommands[cmd][1] ) || ( ( op == '>' || op == '<' ) && !validCommands[cmd][2] ) ) {
          console.error( MCat.invalidTokenMsg + query[token] + MCat.skippingMsg  );
        }
        else {
          if ( seen[cmd] )
            continue;
          ...
          if ( cmd == 'event' ) {
            ...
            if ( usingDateFilter ) {
              console.error( MCat.noEventFilterMsg  );
            ...
            if ( usingEventFilter ) {
              console.error( MCat.noDateFilterMsg  );
        ...
        console.error( MCat.invalidTokenMsg + query[token] );
      }
    }
