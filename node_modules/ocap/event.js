...
function Event( net, finishHandler, emitter ) {
  ...
  self.getDetails = function ( connectionId, connection, finishOverride ) {
    ...
    self.net.get( 'events/' + userType + '/' + connection.org_id,
      function ( response ) {
        ...
        if ( self.isDebugging )
          emitter.emit( 'debugMessage', instance   + ' - ' + MCat.eventsDetail );
        ...
        for ( ce in connection.events ) {
          ...
          if ( !eventFound ) {
            if ( self.isDebugging )
              emitter.emit( 'debugMessage', MCat.deletingEvent + ce );
        ...
        for ( e in events ) {
          ...
          if ( self.isDebugging ) {
            emitter.emit( 'debugMessage', instance + ' - ' + MCat.foundEvent + eventId );
          }
          for ( key in theEvent ) {
            if ( /_time$/.test( key ) )
          ...
          if ( self.isDebugging ) {
            emitter.emit( 'debugMessage', instance + ' - #' + eventId + ': ' + MCat.countdown + ': ' + startsIn );
            emitter.emit( 'debugMessage', instance + ' - #' + eventId + ': ' + MCat.duration  + ': ' + duration );
            emitter.emit( 'debugMessage', instance + ' - #' + eventId + ': ' + MCat.newCheck  + ': ' + newCheckDate );
            emitter.emit( 'debugMessage', instance + ' - #' + eventId + ': ' + MCat.payThru   + ': ' + payThruDate );
            emitter.emit( 'debugMessage', instance + ' - #' + eventId + ': ' + MCat.statusPrefix + ': ' + eventStatus );
          }
        }
        if ( finishOverride )
          finishOverride( connectionId );
      ...
      if ( connection.user_type == MCat.BUYER ) {
        ...
        self.net.post( payload, 'events/' + eventId + '/' + supplierId + '/basket/' + basketId + '/status',
          function ( response ) {
            ...
            if ( self.isDebugging )
              emitter.emit( 'debugMessage', instance + ' - ' + MCat.userDetails );
            var payload              = JSON.parse( response.body ).payload;
            connection.lastActive    = new Date( payload.created );
        ...
        emitter.emit( 'failOCAPservice', instance + ' - ' + MCat.errorSupplierCant );
      }
    }
    else {
      emitter.emit( 'failOCAPservice', MCat.errorNoConnection );
    }
  };
