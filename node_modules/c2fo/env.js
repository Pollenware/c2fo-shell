var MCat     = require( 'message-catalog/c2fo/env.js' ).MCat,
...
function Env( version, authHandler, finishHandler ) {
  ...
  var initState = {
    ...
    isEmulatingInstance : ( p.emulating && p.instance && !p.user ), 
    isEmulatingUser     : ( p.emulating && p.user && !p.instance ), 
    isEmulatingReady    : ( p.emulating && p.instance && p.user ), 
    hasInstanceAndUser  : ( p.user      && p.instance && !p.emulating ),
    hasEmulatingOnly    : ( p.emulating && !p.user ),
    hasUserOnly         : ( p.user      && !p.instance && !p.emulating ),
    hasInstanceOnly     : ( p.instance  && !p.user && !p.emulating )
  };
  if ( initState.isInvalidEmulator ) {
    finishHandler(null, MCat.nonAdminWarning );
  }
  else if ( initState.isEmulatingReady || initState.hasInstanceAndUser ) {
    process.stdin.resume();
    if ( p.emulating )
      this.promptForPassword( p.user + '@' + p.instance + '@' + p.emulating );
    else
      this.promptForPassword( p.user + '@' + p.instance );
  }
