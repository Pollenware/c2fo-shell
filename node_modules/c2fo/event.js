var MCat = require('message-catalog/c2fo/event.js').MCat,
    ...
    self.lockBasket = function (connectionId, connection, eventId, supplierId, basketId, statusCode) {
        ...
        if (connection) {
            if (connection.user_type == MCat.BUYER) {
                ...
                emitter.emit('failedC2FOservice', instance + ' - ' + MCat.errorSupplierCant);
            }
        }
        else {
            emitter.emit('failedC2FOservice', MCat.errorNoConnection);
        }
    };

    self.net = net;

    self.placeOffer = function (connectionId, connection, basket, isBidAgent) {
        ...
        self.net.post(payload, 'events/user/' + connection.user_id + '/' + endpoint,
                function (response) {
                    ...
                    var payload = JSON.parse(response.body).payload;
                    if (self.isDebugging) {
                        emitter.emit('debug', instance + ' - ' + payload.response.updated);
