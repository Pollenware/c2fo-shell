...
function Commands ( env, emitter, Config, MCat, prompt, command, 
  ...
  if ( shell ) {
    ...
    //
      ...
      if (  /^\s*(c|connections)\s+[\@a-zA-Z0-9\-_]+$/.test( command ) ) {
        ...
        if ( Util.isEmpty( shell.env.listConnections() ) )
          emitter.emit( 'commandFail', MCat.failNoConnection );
        else
    ...
    //
      ...
      MCat.help( 
        function () {
          emitter.emit( 'commandFail' );
        },
        function ( failMsg ) {
          emitter.emit( 'commandComplete', failMsg );
        }
      );
    }
    ...
    //
      ...
      emitter.emit( 'commandComplete', null, MCat.replUse + MCat.replAddress + Config.replPort );
    }
    ...
    //
      ...
      //
        ...
        if ( instruction == 'c' || instruction == 'clear' ) {
          ...
          emitter.emit( 'commandComplete', null, logText );
        } 
      }
  ...
  //
    ...
    shell.env.promptForPassword( command, keypressListener );
  }
