var Config  = require( 'config/commands.js' ).Config,
  Connector = require( 'connector.js' ).Connector,
  MCat      = require( 'message-catalog/commands.js' ).MCat,
  net       = require( 'net' ),
  repl      = require( 'repl' ),
...
var basketCount = 0;
// {"org_id":209161,"buyer_id":209161,"start_time":"2011-11-10T20:13:00-06:00","end_time":"2011-11-10T21:13:00-06:00","pay_thru_date":"2011-11-10T00:00:00-06:00","new_check_date":"2011-11-10T00:00:00-06:00","cash_pool":653490.93,"configurations":[{"max":10,"starting_bid":0.05,"bid_increment":0.05,"hurdle_rate":10,"is_discount_based":false,"min":null,"title":"1-10 Days","type":0},{"max":30,"starting_bid":0.05,"bid_increment":0.05,"hurdle_rate":10,"is_discount_based":false,"min":11,"title":"11-30 Days","type":1},{"max":45,"starting_bid":0.05,"bid_increment":0.05,"hurdle_rate":10,"is_discount_based":false,"min":31,"title":"31-45 Days","type":2},{"max":60,"starting_bid":0.05,"bid_increment":0.05,"hurdle_rate":10,"is_discount_based":false,"min":46,"title":"46-60 Days","type":3}]}
var eventBuffer = {};
eventBuffer.configurations = [];
function Commands ( env, emitter, prompt, command, 
                    shell, library, connectionId, connection ) {
  ...
  if ( shell ) {
    ...
    // scheduling an event
    // 
    if ( shell.env.context.scheduling ) {

      var s = shell.env.context.scheduling;

      // finish baskets
      if ( command == '.' ) {
        basketCount = 0;
        delete shell.env.context.scheduling;
        eventBuffer.configurations.pop();
        emitter.emit( 'completedCommand', connectionId, JSON.stringify( eventBuffer ) );
      }
      else if ( s.baskets && s.finished ) {
        eventBuffer.configurations[basketCount].hurdle_rate = parseInt( command.trim() );
        s.finished = false; 

        // start new basket
        basketCount++;
        eventBuffer.configurations[basketCount] = {};
        emitter.emit( 'schedulePrompt', MCat.minDaysPrompt, "\n" + MCat.dotMessage + "\n\n" + MCat.basketNumLabel + ( basketCount + 1 ) );
        s.baskets = true; s.askMax = true; s.cash_pool = false;
      }
      else if ( s.start_time ) {
        eventBuffer.start_time = Util.iso8601Date( command.trim() );
        eventBuffer.start_time_ms = new Date( command.trim() ).getTime();
        emitter.emit( 'schedulePrompt', MCat.durationPrompt, null );
        s.duration = true; s.start_time = false;
      }
      else if ( s.duration ) {
        eventBuffer.end_time = Util.iso8601Date( new Date( eventBuffer.start_time_ms + ( parseInt( command.trim() )  * 60 * 1000 ) ) ) ;
        delete eventBuffer.start_time_ms;
        delete eventBuffer.duration;
        emitter.emit( 'schedulePrompt', MCat.payThruPrompt, null );
        s.payThru = true; s.duration = false;
      }
      else if ( s.payThru ) {
        eventBuffer.pay_thru_date = Util.iso8601Date( command.trim() );
        emitter.emit( 'schedulePrompt', MCat.newCheckPrompt, null );
        s.newCheck = true; s.payThru = false;
      }
      else if ( s.newCheck ) {
        eventBuffer.new_check_date = Util.iso8601Date( command.trim() );
        emitter.emit( 'schedulePrompt', MCat.cashPoolPrompt, null );
        s.cash_pool = true; s.newCheck = false;
      }
      else if ( s.cash_pool ) {
        eventBuffer.cash_pool = parseInt( command.trim() );

        // begin baskets
        eventBuffer.org_id = connection.org_id; 
        eventBuffer.buyer_id = connection.org_id; 
        eventBuffer.configurations[basketCount] = {};
        eventBuffer.configurations[basketCount].type = parseInt( basketCount );
        emitter.emit( 'schedulePrompt', MCat.minDaysPrompt, MCat.basketNumLabel + (basketCount + 1 ) );
        s.baskets = true; s.askMax = true; s.cash_pool = false;
      }
      else if ( s.baskets && s.askMax ) {
        var minDays = command.trim();
        if ( minDays == 'e' )
          minDays = null;  
        else
          minDays = parseInt( minDays )
        eventBuffer.configurations[basketCount].min = minDays;
        emitter.emit( 'schedulePrompt', MCat.maxDaysPrompt, null );

        s.askMax = false; s.startOffer = true;
      }
      else if ( s.baskets && s.startOffer ) {
        var maxDays = command.trim();
        if ( maxDays == 'L' )
          maxDays = null;
        else
          maxDays = parseInt( maxDays );
        eventBuffer.configurations[basketCount].max = maxDays;
        eventBuffer.configurations[basketCount].title = eventBuffer.configurations[basketCount].min + 
          MCat.daysSeparator + eventBuffer.configurations[basketCount].max + MCat.basketSuffix;
        emitter.emit( 'schedulePrompt', MCat.startOfferPrompt, null );
        s.startOffer = false; s.increment = true;
      }
      else if ( s.baskets && s.increment ) {
        eventBuffer.configurations[basketCount].starting_bid = parseFloat( command.trim() );
        emitter.emit( 'schedulePrompt', MCat.incrementPrompt, null );
        s.increment = false; s.hurdleType = true;
      }
      else if ( s.baskets && s.hurdleType ) {
        eventBuffer.configurations[basketCount].bid_increment = parseFloat( command.trim() );
        emitter.emit( 'schedulePrompt', MCat.hurdleTypePrompt, null );
        s.hurdleType = false; s.hurdle = true;
      }
      else if ( s.baskets && s.hurdle ) {
        var basketType = command.trim();
        if ( basketType == 'apr' || basketType == 'a' ) {
          eventBuffer.configurations[basketCount].is_discount_based = false;
        }
        else if ( basketType == 'discount'  || basketType == 'd' ) {
          eventBuffer.configurations[basketCount].is_discount_based = true;
        }
        else {
          emitter.emit( 'failedCommand', MCat.hurdleTypePrompt, MCat.failedBasketType );
          return;
        }
        emitter.emit( 'schedulePrompt', MCat.hurdlePrompt, null );
        s.hurdle = false; s.finished = true;
      }
      return;
    }
    ...
    //
      ...
      if (  /^\s*(c|connections)\s+[\@a-zA-Z0-9\-_]+$/.test( command ) ) {
        ...
        if ( !found ) {
          emitter.emit( 'failedCommand', MCat.notFound );
        }
      }
      else if ( /^\s*(c|connections)\s*$/.test( command ) ) {
        found = true;
        if ( Util.isEmpty( shell.env.listConnections() ) )
          emitter.emit( 'failedCommand', MCat.failedNoConnection );
        else
    ...
    //
      ...
      MCat.help( 
        function () {
          emitter.emit( 'failedCommand' );
        },
        function ( msg ) {
          emitter.emit( 'completedCommand', null, msg );
        }
      );
    }
    ...
    //
      ...
      var server = net.createServer( function ( socket ) {
        ...
        repl.start( MCat.replPrompt + MCat.promptPoint, socket, null, false ).context.shell = shell;
      });
      server.listen( Config.replPort || 5100 )
      server.on( 'error', function ( e ) {
        emitter.emit( 'failedCommand', e );
      });
      emitter.emit( 'completedCommand', null, MCat.replUse + MCat.replAddress + Config.replPort );
    }
    ...
    //
      ...
      //
        ...
        if ( connection.user_type == MCat.SUPPLIER ) {
          ...
          emitter.emit( 'failedCommand', MCat.failedNoBuyer );
      }
        ...
        if ( eventId && id ) {
          ...
          try {
            if ( connection.events[eventId].baskets ) {
              ...
              emitter.emit( 'failedCommand', MCat.noBaskets );
            }
          }
          catch ( err ) {
            emitter.emit( 'failedCommand', MCat.notFound );
      ...
      //
        ...
        if ( instruction == 'c' || instruction == 'clear' ) {
          ...
          emitter.emit( 'completedCommand', null, logText );
        } 
      }
      ...
      //
        ...
        if ( turnOff == 'off' ) {
          if ( library.event.timers[connection.instance] && library.event.timers[connection.instance][requestedEvent] ) {
            ...
            emitter.emit( 'failedCommand', MCat.failedEventNotFound + ': '  + requestedEvent );
          }
        }
        else {
          if ( library.event.timers[connection.instance] && library.event.timers[connection.instance][requestedEvent] ) {
            ...
            if ( sanitizedConnection && sanitizedConnection.events) {
              ...
              if ( eventFound ) {
                ...
                var monitorEvent = function () {                    
                  ...
                  }                                                   // overridding successHandler to not change prompt
                  library.event.getDetails( connectionId, connection, function () {
                    emitter.emit( 'completedCommand', null, displayMsg );
                  } );
                };
              ...
              emitter.emit( 'failedCommand', MCat.failedNoEvents );
            }
          }
        }
        emitter.emit( 'completedCommand' );
      ...
      //
        ...
        if ( !eventFound ) {
          emitter.emit( 'failedCommand', MCat.failedEventNotFound );
        }
        else {
          if ( env.context.debug ) 
          ...
          if ( connection.user_type == MCat.BUYER ) 
            ...
            emitter.emit( 'failedCommand', MCat.failedNoSupplier );
        }
      }
      ...
      //
        ...
        if ( !connection.events ) {
          ...
          emitter.emit( 'failedCommand', MCat.failedPrefix + MCat.s + MCat.failedBuyerBid );
          return;
        }
      ...
      // schedule - s
      //
      else if ( /^\s*(s|schedule)\s*$/.test( command ) ) {
        shell.env.context.scheduling = {};
        shell.env.context.scheduling.start_time = true;
        emitter.emit( 'schedulePrompt', MCat.startTimePrompt, null );
      }

      //
      // toggle - t (invoice inclusion)
      //
        ...
        if ( connection ) {
          ...
          for ( var e in connection.events ) {
            if (  connection.events[e].is_live || connection.events[e].is_buyer_live ) {
              emitter.emit( 'failedCommand', MCat.failedLiveEvent );
              return;
            }
          }
          ...
          for ( var i in invoiceIds ) {
            ...
            if (! /^[ieEI][\|\d]+$/.test( iId ) ) {
              emitter.emit( 'failedCommand', MCat.failedAmbiguous + iId );
              continue;
            }
            else {
              if ( /^i/i.test( iId ) ) {
  ...
  //
  // only commands supported w/o authentication are connection string,
  // h|help and q|quit
  //
    ...
    var keypressListener = function( s, key ) {
      shell.prompt.readline._ttyWrite( s, key );
    };
  ...
  //
    ...
    MCat.help( 
      ...
      function ( failedMsg ) {
        emitter.emit( 'completedCommand', failedMsg );
     }
   );
  }
