  ...
  if ( shell ) {
    ...
    if ( shell.env.context.scheduling ) {
      ...
      if ( command == '.' ) {
        ...
        if ( minDays == 'e' ) {
          minDays = Config.earliestPossible;  
        }
        eventBuffer.configurations[basketCount].min = parseInt( minDays );
        emitter.emit( 'schedulePrompt', MCat.maxDaysPrompt, null );

        s.askMax = false; s.startOffer = true;
      }
      else if ( s.baskets && s.startOffer ) {
        eventBuffer.configurations[basketCount].max = parseInt( command.trim() );
