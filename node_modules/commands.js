...
function Commands(env, emitter, prompt, command, shell, library, connectionId, connection) {
    ...
    if (shell) {
        ...
        //
                    ...
                    for (var c in connections) {
                        if (connections[c].id == thisConnectionId) {
                            ...
                            if (c == shell.env.connectionString()) {
                                shell.prompt.content = function () {
                                    return MCat.promptPrefix + MCat.promptPoint
                                };
                                shell.prompt.set();
                            }
                        }
            ...
            //
            else if (/^\s*(o|offer)\s+\d+\s+\d+\s+\d+\s*(t|f)$/.test(command)) {

                command = command.split(/\s+/);
                var eventId = command[1] && command[1].trim();
                var basket = command[2] && command[2].trim();
                var bps = command[3] && command[3].trim();
                var isBidAgent = command[4] && command[4].trim();
                if (!connection.events) {
                    ...
                    if (connection.events[eventId]) {
                        ...
                        if (found) {
                            ...
                            isBidAgent = isBidAgent == 't';
                            library.event.placeOffer(connectionId, connection, thisBasket, isBidAgent);
                        }
                        else {
                            emitter.emit('failedCommand', MCat.notFound);
                        }
